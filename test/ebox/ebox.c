...
void eboxtest(char *ifname)
{
  ...
  //          out_EBOX = (out_EBOX_streamt*) ec_slave[1].outputs;
            ...
            {
               printf("Slave:%d Name:%s State:%2d delay:%d.%d\n",
                     cnt, ec_slave[cnt].name, ec_slave[cnt].state, (int)ec_slave[cnt].pdelay, ec_slave[cnt].hasdc);
            }
            ...
            if (ec_slave[0].state == EC_STATE_OPERATIONAL )
            {
               ...
               for(i = 1; i <= 20000; i++)
               {
                  ...
                  printf("PD cycle %5d DCtime %12lld DCdiff=%d\n", cyclecount, ec_DCtime, DCdiff);
